export default class Zoomify {
  ...
  zoomIn() {
    ...
    elm.style.transition = `scale ${this.config.transitionDuration}ms ease-in-out`;
  ...
  enableZoom(value) {
      this.zoom = value
      value ? this.zoomIn() : this.zoomOut()
  }
  ...
  setZoomEvents(detach = false) {
    ...
    ;['mousemove','touchmove'].forEach((name) => {
        if (detach) {
            elm.removeEventListener(name, this.focusZoom)
        } else {
            elm.addEventListener(name, this.focusZoom, { passive: true })
        }
    })
    ...
    // document.querySelector(this.config.selector).zoomify = this
  }
